...
- Keep scheduler business logic in `src/services/SchedulerService.php`; scheduler CLI/CP controllers are thin wrappers around this service, while rule-file CRUD lives in `src/controllers/RuleFileController.php`.
...
- `buildRows()` also supports rule entries with `action: "ignore"`, which remove already-accumulated rows for matching variants on the same `effective_date`.
...
- `PriceSchedule::beforeSave()` appends `updateHistory` entries (`userId`, `timestamp`, `newPrice`, `newPromotionalPrice`) on each save; preserve this when changing write paths.
...
- CP utility stack:
  ...
  - server (rule files): `src/utilities/RuleFileUtility.php`, `src/controllers/RuleFileController.php`
...
- Rule-file form actions to keep in sync with controller routes:
  - `_priceadjuster/rule-file/save`
  - `_priceadjuster/rule-file/duplicate`
  - `_priceadjuster/rule-file/delete`
